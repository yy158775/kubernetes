...
// WithAllowedRedirectionHosts adds the given value to the AllowedRedirectionHosts field in the declarative configuration
// and returns the receiver, so that objects can be build by chaining "With" function invocations.
// If called multiple times, values provided by each call will be appended to the AllowedRedirectionHosts field.
func (b *RuleApplyConfiguration) WithAllowedRedirectionHosts(values ...string) *RuleApplyConfiguration {
	for i := range values {
		b.AllowedRedirectionHosts = append(b.AllowedRedirectionHosts, values[i])
	}
	return b
}
